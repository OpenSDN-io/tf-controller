...
TestTag src[] = {
    {"Tag1", 1, ((uint64_t)1 << TagEntry::kTagTypeBitShift) | 1},
    {"Tag2", 2, ((uint64_t)2 << TagEntry::kTagTypeBitShift) | 2},
    {"Tag13", 13, 13}
};

TestTag dst[] = {
    {"Tag3", 3, ((uint64_t)1 << TagEntry::kTagTypeBitShift) | 3},
    {"Tag4", 4, ((uint64_t)2 << TagEntry::kTagTypeBitShift) | 4}
};
...
TEST_F(FirewallPolicy, Test7) {
    ...
    packet1->src_tags_.push_back((uint64_t)1 << TagEntry::kTagTypeBitShift | 0x2);
    packet1->dst_tags_.push_back((uint64_t)1 << TagEntry::kTagTypeBitShift | 0x3);
    EXPECT_FALSE(acl->PacketMatch(*packet1, m_acl, NULL));

    packet1->src_tags_.clear();
    packet1->dst_tags_.clear();
    packet1->src_tags_.push_back((uint64_t)1 << TagEntry::kTagTypeBitShift | 0x2);
    packet1->dst_tags_.push_back((uint64_t)1 << TagEntry::kTagTypeBitShift | 0x2);
    EXPECT_TRUE(acl->PacketMatch(*packet1, m_acl, NULL));
    ...
    packet1->src_tags_.push_back((uint64_t)2 << TagEntry::kTagTypeBitShift | 0x1);
    packet1->dst_tags_.push_back((uint64_t)2 << TagEntry::kTagTypeBitShift | 0x1);
    std::sort(packet1->src_tags_.begin(), packet1->src_tags_.end());
    std::sort(packet1->dst_tags_.begin(), packet1->dst_tags_.end());
    EXPECT_TRUE(acl->PacketMatch(*packet1, m_acl, NULL));

    packet1->src_tags_[1] = ((uint64_t)2 << TagEntry::kTagTypeBitShift | 0x2);
    packet1->dst_tags_[1] = ((uint64_t)2 << TagEntry::kTagTypeBitShift | 0x1);
    packet1->src_tags_.push_back((uint64_t)2 << TagEntry::kTagTypeBitShift | 0x3);
    packet1->dst_tags_.push_back((uint64_t)2 << TagEntry::kTagTypeBitShift | 0x3);
    EXPECT_TRUE(acl->PacketMatch(*packet1, m_acl, NULL));
