/*
 * Copyright (c) 2025 Elena Zizganova
 */
...
public:
    ...
    void set_state_machine_restart(bool flag) {
        state_machine_restart_ = flag;
    }
...
class BgpaasBaseVxlan {
protected:
    BgpaasBaseVxlan() :
            server_session_manager_(nullptr), vm1_session_manager_(nullptr),
            vm2_session_manager_(nullptr) {
    }

    void SetUp(int vm1_port, int vm2_port) {
    ...
    void SetUpAgent() {
        ...
        agent_->SubscribeAll("test_vxlan", 123);
    }
    ...
    void XmppShutdown() {
        ...
        xmpp_server_ = nullptr;
    }

    BgpPeerTest *FindPeer(BgpServerTest *server, const char *instance_name,
                          const string &uuid) {
        TASK_UTIL_EXPECT_NE(static_cast<BgpPeerTest *>(nullptr),
    ...
    void VerifyBgpaasRTargetRoutes(const BgpServerTest *server,
        ...
        const RoutingInstance *rtinstance =
            static_cast<const RoutingInstance *>(
    ...
    void CheckBgpaasRTargetRoutePresence(const BgpTable *table,
        ...
        const RTargetTable::RequestKey key(
            RTargetPrefix::FromString("64512:target:64512:100", nullptr), peer);
    ...
    bool CheckBgpaasRTargetRoutePresence(const BgpTable *table,
        ...
        bool result = false;
        task_util::TaskFire(
            boost::bind(&BgpaasBaseVxlan::CheckBgpaasRTargetRoutePresence,
                        this, table, peer, &result), "bgp::Config");
        return result;
    }
    ...
    bool CheckBgpaasRTargetRouteAbsence(const BgpTable *table,
        ...
        bool result = false;
        task_util::TaskFire(
            boost::bind(&BgpaasBaseVxlan::CheckBgpaasRTargetRouteAbsence,
                        this, table, peer, &result), "bgp::Config");
        return result;
    }
    ...
    boost::scoped_ptr<BgpInstanceConfigTest> test_vxlan_cfg_;
...
class BgpaasVxlanTest : public BgpaasBaseVxlan,
...
protected:
    void SetUp() {
        ...
        BgpaasBaseVxlan::SetUp(0, 0);
    }

    void TearDown() {
        BgpaasBaseVxlan::TearDown();
    }
    ...
    void PrintTable(const BgpTable *table) const {
        ...
        DBTable::DBTableWalkRef walk_ref =
            const_cast<BgpTable *>(table)->AllocWalker(
                boost::bind(&BgpaasVxlanTest::WalkCallback, this, _1, _2),
                boost::bind(&BgpaasVxlanTest::WalkDoneCallback, this, _1, _2,
                            &complete));
    ...
    void AddBgpInetRoute(BgpServerTest *server,
        ...
        req.key.reset(new InetTable::RequestKey(prefix, nullptr));
    ...
    void DeleteBgpInetRoute(BgpServerTest *server, std::string prefix_str) {
        Ip4Prefix prefix(Ip4Prefix::FromString(prefix_str));
    ...
    void AddBgpInet6Route(BgpServerTest *server, std::string prefix_str,
        ...
        DBRequest req;
        req.oper = DBRequest::DB_ENTRY_ADD_CHANGE;
        req.key.reset(new Inet6Table::RequestKey(prefix, nullptr));
        BgpAttrPtr attr = server->attr_db()->Locate(attr_spec);
    ...
    bool VerifyInetRoutePresenceActual(BgpServer *server,
        ...
        BgpTable *table = rtinstance->GetTable(Address::INET);
        Ip4Prefix prefix(Ip4Prefix::FromString(prefix_str));
        const InetTable::RequestKey key(prefix, nullptr);
        if (!table->Find(&key))
            return false;

        // BGPaaS is _not_ supported for ibgp. Just for completeness, this test
        // also tests ibgp bgpaas sessions bringup part. As-path is not
        // modified for ibgp either. Also, nexthop is not modified and
        // resolved. All the test parameters passed in here are only applicable
        // to ebgp.
        if (!ebgp_)
            return true;
    ...
    void VerifyInetRouteAbsence(BgpServer *server,
        ...
        Ip4Prefix prefix(Ip4Prefix::FromString(prefix_str));
        const InetTable::RequestKey key(prefix, nullptr);
        TASK_UTIL_EXPECT_EQ(static_cast<const BgpRoute *>(nullptr),
                            table->Find(&key));
    }
    ...
    bool VerifyInet6RoutePresenceActual(BgpServer *server,
        ...
        BgpTable *table = rtinstance->GetTable(Address::INET6);
        Inet6Prefix prefix(Inet6Prefix::FromString(prefix_str));
        const Inet6Table::RequestKey key(prefix, nullptr);
        if (!table->Find(&key))
            return false;
        if (!ebgp_)
            return true;
    ...
    void VerifyEvpnRouteCount(BgpServer *server, size_t expected) {
        RoutingInstance *rtinstance = static_cast<RoutingInstance *>(
            server->routing_instance_mgr()->GetRoutingInstance(
                BgpConfigManager::kMasterInstance));
        BgpTable *table = rtinstance->GetTable(Address::EVPN);
        TASK_UTIL_EXPECT_EQ(expected, table->Size());
    }

    bool VerifyEvpnRoutePresenceActual(BgpServer *server,
                                       const std::string &prefix_str,
                                       const std::string &nexthop_str,
                                       const std::string &as_path = "") {
        task_util::TaskSchedulerLock lock;
        ...
        const EvpnTable::RequestKey key(prefix, nullptr);
        if (!table->Find(&key))
            return false;
    ...
    void VerifyEvpnRoutePresence(BgpServer *server,
                                 const std::string &prefix_str,
                                 const std::string &nexthop_str,
                                 const std::string &as_path = "") {
        TASK_UTIL_EXPECT_TRUE(VerifyEvpnRoutePresenceActual(
            server, prefix_str, nexthop_str, as_path));
    }

    void VerifyEvpnRouteAbsence(BgpServer *server,
                                const std::string &prefix_str) {
        RoutingInstance *rtinstance = static_cast<RoutingInstance *>(
            server->routing_instance_mgr()->GetRoutingInstance(
                BgpConfigManager::kMasterInstance));
    ...
    bool VerifyEnetRoutePresenceActual(test::NetworkAgentMock *agent,
                                 std::string prefix, std::string nexthop1,
                                 std::string nexthop2 = "") {
        task_util::TaskSchedulerLock lock;
        if (!agent->EnetRouteLookup("test_vxlan", prefix))
            return false;
        const test::NetworkAgentMock::EnetRouteEntry *item_evpn =
            agent->EnetRouteLookup("test_vxlan", prefix);
        size_t nexthop_count = nexthop2.empty() ? 1 : 2;
        if (nexthop_count != item_evpn->entry.next_hops.next_hop.size())
            return false;
        if (nexthop1 != item_evpn->entry.next_hops.next_hop[0].address)
            return false;
        if (!nexthop2.empty()) {
            if (nexthop2 != item_evpn->entry.next_hops.next_hop[1].address)
                return false;
        }
        return true;
    }

    void VerifyEnetRoutePresence(test::NetworkAgentMock *agent,
                                 std::string prefix, std::string nexthop1,
                                 std::string nexthop2 = "") {
        TASK_UTIL_EXPECT_TRUE(VerifyEnetRoutePresenceActual(agent, prefix,
                                  nexthop1, nexthop2));
    }
    ...
    void VerifyInetRouteAbsence(test::NetworkAgentMock *agent,
                                std::string prefix) {
        TASK_UTIL_EXPECT_EQ(
            static_cast<const test::NetworkAgentMock::RouteEntry *>(nullptr),
            agent->RouteLookup("test", prefix));
    }
    ...
    void VerifyEnetRouteAbsence(test::NetworkAgentMock *agent,
                                std::string prefix) {
        TASK_UTIL_EXPECT_EQ(
            static_cast<const test::NetworkAgentMock::EnetRouteEntry *>
                (nullptr), agent->EnetRouteLookup("test_vxlan", prefix));
    }

    void RunTest();
};

void BgpaasBaseVxlan::InitializeTemplates(bool set_local_as,
...
void BgpaasBaseVxlan::SetUpControlNodes() {
    ...
    test_vxlan_cfg_.reset(BgpTestUtil::CreateBgpInstanceConfig("test_vxlan",
                "target:64512:123", "target:64512:123"));
    TaskScheduler *scheduler = TaskScheduler::GetInstance();
    scheduler->Stop();
    server_->routing_instance_mgr()->
        CreateRoutingInstance(test_vxlan_cfg_.get());

    RoutingInstance *ri_test =
        server_->routing_instance_mgr()->GetRoutingInstance("test");
    ri_test->set_routing_instance_vxlan("test_vxlan");
    scheduler->Start();
    task_util::WaitForIdle();
...
void BgpaasVxlanTest::RunTest() {
    ...
    TASK_UTIL_EXPECT_EQ(0, agent_->enet_route_mgr_->Count());
    VerifyEnetRouteAbsence(agent_.get(), "00:00:00:00:00:00,1.1.1.1/32");
    VerifyEnetRouteAbsence(agent_.get(), "00:00:00:00:00:00,1.1.1.2/32");
    VerifyEnetRouteAbsence(agent_.get(), "00:00:00:00:00:00,1.1.1.3/32");
    VerifyEnetRouteAbsence(agent_.get(), "00:00:00:00:00:00,20.20.20.1/32");
    VerifyEnetRouteAbsence(agent_.get(), "00:00:00:00:00:00,20.20.20.2/32");
    VerifyEnetRouteAbsence(agent_.get(), "00:00:00:00:00:00,dead:1::beef/128");
    VerifyEnetRouteAbsence(agent_.get(), "00:00:00:00:00:00,dead:2::beef/128");
    ...
    agent_->AddEnetRoute("test_vxlan", "00:00:00:00:00:00,1.1.1.1/32",
                         "10.10.10.1");
    task_util::WaitForIdle();
    ...
    TASK_UTIL_EXPECT_EQ(3, agent_->enet_route_mgr_->Count());
    VerifyEnetRoutePresence(agent_.get(), "00:00:00:00:00:00,1.1.1.1/32",
                            "10.10.10.1");
    VerifyEnetRoutePresence(agent_.get(), "00:00:00:00:00:00,20.20.20.1/32",
                            "10.10.10.1");
    VerifyEnetRouteAbsence(agent_.get(), "00:00:00:00:00:00,20.20.20.2/32");
    VerifyEnetRoutePresence(agent_.get(), "00:00:00:00:00:00,dead:1::beef/128",
                            "10.10.10.1");
    VerifyEnetRouteAbsence(agent_.get(), "00:00:00:00:00:00,dead:2::beef/128");
    ...
    agent_->AddEnetRoute("test_vxlan", "00:00:00:00:00:00,1.1.1.2/32",
                         "10.10.10.2");
    agent_->AddEnetRoute("test_vxlan", "00:00:00:00:00:00,1.1.1.3/32",
                         "10.10.10.3");
    task_util::WaitForIdle();
    ...
    TASK_UTIL_EXPECT_EQ(7, agent_->enet_route_mgr_->Count());
    VerifyEnetRoutePresence(agent_.get(), "00:00:00:00:00:00,1.1.1.1/32",
                            "10.10.10.1");
    VerifyEnetRoutePresence(agent_.get(), "00:00:00:00:00:00,1.1.1.2/32",
                            "10.10.10.2");
    VerifyEnetRoutePresence(agent_.get(), "00:00:00:00:00:00,1.1.1.3/32",
                            "10.10.10.3");
    VerifyEnetRoutePresence(agent_.get(), "00:00:00:00:00:00,20.20.20.1/32",
                            "10.10.10.1");
    VerifyEnetRoutePresence(agent_.get(), "00:00:00:00:00:00,20.20.20.2/32",
                            "10.10.10.2");
    VerifyEnetRoutePresence(agent_.get(), "00:00:00:00:00:00,dead:1::beef/128",
                            "10.10.10.1");
    VerifyEnetRoutePresence(agent_.get(), "00:00:00:00:00:00,dead:2::beef/128",
                            "10.10.10.3");
    ...
    VerifyInet6RouteCount(vm2_.get(), 3);
    TASK_UTIL_EXPECT_EQ(8, agent_->enet_route_mgr_->Count());
    ...
    // GR session would have either got refreshed or deleted, based on the
    // timer expiry and when the session comes up. Most of time time, we
    // expect the session to be swept. But add delete check to make test more
    // stable.
    ...
    agent_->AddEnetRoute("test_vxlan", "00:00:00:00:00:00,3.1.1.1/32",
                         "30.10.10.2");
    agent_->AddEnetRoute("test_vxlan", "00:00:00:00:00:00,4.1.1.1/32",
                         "40.10.10.2");
    task_util::WaitForIdle();
    ...
    TASK_UTIL_EXPECT_EQ(11, agent_->enet_route_mgr_->Count());
    VerifyEnetRoutePresence(agent_.get(), "00:00:00:00:00:00,3.1.1.1/32",
                            "30.10.10.2");
    VerifyEnetRoutePresence(agent_.get(), "00:00:00:00:00:00,4.1.1.1/32",
                            "40.10.10.2");
    VerifyEnetRoutePresence(agent_.get(), "00:00:00:00:00:00,30.20.20.1/32",
                            "30.10.10.2", "40.10.10.2");
    VerifyEnetRoutePresence(agent_.get(), "00:00:00:00:00:00,feed:2::feed/128",
                             "30.10.10.2", "40.10.10.2");
    ...
    agent_->DeleteEnetRoute("test_vxlan", "00:00:00:00:00:00,3.1.1.1/32");
    agent_->DeleteEnetRoute("test_vxlan", "00:00:00:00:00:00,4.1.1.1/32");
    ...
    TASK_UTIL_EXPECT_EQ(7, agent_->enet_route_mgr_->Count());
    VerifyEnetRouteAbsence(agent_.get(), "00:00:00:00:00:00,3.1.1.1/32");
    VerifyEnetRouteAbsence(agent_.get(), "00:00:00:00:00:00,4.1.1.1/32");
    VerifyEnetRouteAbsence(agent_.get(), "00:00:00:00:00:00,30.20.20.1/32");
    VerifyEnetRouteAbsence(agent_.get(), "00:00:00:00:00:00,feed:2::feed/128");

    // Delete agent route to make bgp route's nexthop not resolvable any more.
    agent_->DeleteRoute("test", "1.1.1.1/32");
    agent_->DeleteEnetRoute("test_vxlan", "00:00:00:00:00:00,1.1.1.1/32");
    ...
    TASK_UTIL_EXPECT_EQ(4, agent_->enet_route_mgr_->Count());
    VerifyEnetRoutePresence(agent_.get(), "00:00:00:00:00:00,1.1.1.2/32",
                            "10.10.10.2");
    VerifyEnetRoutePresence(agent_.get(), "00:00:00:00:00:00,1.1.1.3/32",
                            "10.10.10.3");
    ...
    agent_->DeleteEnetRoute("test_vxlan", "00:00:00:00:00:00,1.1.1.2/32");
    agent_->DeleteEnetRoute("test_vxlan", "00:00:00:00:00:00,1.1.1.3/32");
...
TEST_P(BgpaasVxlanTest, Basic) {
    ...
    peer_vm1->set_process_session_fnc(
        boost::bind(&BgpaasVxlanTest::ProcessSession, this));
    peer_vm1->set_check_split_horizon_fnc(
        boost::bind(&BgpaasVxlanTest::CheckSplitHorizon, this, 0, 0));
...
class BgpaasVxlanTestPort : public BgpaasBaseVxlan,
                       public ::testing::Test {
public:
    bool ProcessSession() const { return true; }
...
protected:
    ...
    void SetUp() {
        ...
        BgpaasBaseVxlan::SetUp(11036, 10043);
    }
...
TEST_F(BgpaasVxlanTestPort, TestSourcePort) {

// Initialize tempplates with 2 vm's and a server with start and end port for
// bgpaas configure 1 vm with port range less than start port and 1 vm with
// greater check if session is established

    vm1_as_ = "700";
    vm2_as_ = "800";

    BgpaasBaseVxlan::InitializeTemplates(set_local_as_,
                        set_local_as_ ? "700" : "64512",
                        set_local_as_ ? "800" : "64512", set_auth_);
    ...
    BgpPeerTest *peer_vm1 = FindPeer(server_.get(), "test",
            BgpConfigParser::session_uuid("bgpaas-server", "vm1", 1));
    peer_vm1->set_process_session_fnc(
        boost::bind(&BgpaasVxlanTestPort::ProcessSession, this));
    peer_vm1->set_check_split_horizon_fnc(
        boost::bind(&BgpaasVxlanTestPort::CheckSplitHorizon, this));
    ...
    BgpPeerTest *peer = BgpaasBaseVxlan::FindPeer(vm1_.get(),
                        BgpConfigManager::kMasterInstance, uuid);
    BGP_WAIT_FOR_PEER_STATE(peer, StateMachine::ESTABLISHED);

    uuid = BgpConfigParser::session_uuid("bgpaas-server", "vm2", 1);
    BgpPeerTest *peer1 = BgpaasBaseVxlan::FindPeer(vm2_.get(),
                         BgpConfigManager::kMasterInstance, uuid);
...
INSTANTIATE_TEST_CASE_P(BgpaasVxlanTestWithParam, BgpaasVxlanTest,
