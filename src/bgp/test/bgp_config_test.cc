...
TEST_F(BgpConfigTest, BgpAllTagsAreGlobalChange) {
    string content_a =
        FileRead("controller/src/bgp/testdata/config_test_48a.xml");
    EXPECT_TRUE(parser_.Parse(content_a));
    task_util::WaitForIdle();
    TASK_UTIL_EXPECT_FALSE(server_.global_config()->all_tags_are_global());

    string content_b =
        FileRead("controller/src/bgp/testdata/config_test_48b.xml");
    EXPECT_TRUE(parser_.Parse(content_b));
    task_util::WaitForIdle();
    TASK_UTIL_EXPECT_TRUE(server_.global_config()->all_tags_are_global());
