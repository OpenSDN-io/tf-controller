...
class TestAnsibleVpgDM(TestAnsibleCommonDM):
    ...
    def test_lag_config_push(self):
        ...
        vmi, vm, pi_list = self.attach_vmi(str(vxlan_id),
                                           ['xe-0/0/1', 'xe-0/0/2'], [pr, pr],
                                           vn_obj, None, fabric, vlan_tag)

        gevent.sleep(1)
        abstract_config = self.check_dm_ansible_config_push()

        phy_intf = self.get_phy_interfaces(
            abstract_config.get('device_abstract_config').get('features',
                                                              {}).get(
                'l2-gateway', {}), name='ae0')
    ...
    def test_mh_config_push(self):
        ...
        if abstract_config.get(
                'management_ip') == '3.3.3.3' and 'xe-0/0/1' not in link_members:
            self.assertTrue(False)
        if abstract_config.get(
                'management_ip') == '4.4.4.4' and 'xe-0/0/2' not in link_members:
            self.assertTrue(False)
    ...
    def test_lag_sg_feature_based_config_push(self):
        ...
        self.create_features(['overlay-bgp', 'l2-gateway','firewall'])
        self.create_physical_roles(['leaf'])
        self.create_overlay_roles(['crb-access'])
        self.create_role_definitions([
            AttrDict({
                ...
                'physical_role': 'leaf',
                'overlay_role': 'crb-access',
                'features': ['overlay-bgp', 'l2-gateway','firewall'],
                'feature_configs': {}
            })
        ])
        ...
        vmi, vm, pi_list = self.attach_vmi(str(vxlan_id), ['xe-0/0/1',
                                                           'xe-0/0/2'], [pr,
                                                                         pr],
                                           vn_obj, sg_obj, fabric, None,
                                           port_vlan_tag)

        gevent.sleep(1)
        abstract_config = self.check_dm_ansible_config_push()
        print(abstract_config)
        ...
        fw = self.get_firewalls(abstract_config.get('device_abstract_config').get('features', {}).get('firewall', {}))
        if not fw:
            self.assertTrue(False)
