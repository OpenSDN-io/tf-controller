...
class ImportServerJob(object):
    JOB_STATUS_MAPPING = {
        'SUCCESS': ImportServerStatus.COMPLETE,
        'FAILURE': ImportServerStatus.FAILED,
        'UNKNOWN': ImportServerStatus.FAILED
    }
    ...
    def __init__(self, job_type, job_input, api_server_config):
        ...
        self._job_id = None
        self._job_status = ImportServerStatus.INIT
        super(ImportServerJob, self).__init__()
    # end __init__

    def push(self, timeout, max_retries):
        vnc_api = self._get_vnc_api(self._api_server_config)
        self._job_status = ImportServerStatus.IN_PROGRESS
        ...
        try:
            ...
            self._job_status = ImportServerStatus.FAILED

        if self._job_status == ImportServerStatus.FAILED:
    ...
    def _get_job_status(self, vnc_api, job_execution_id):
        if self._check_job_status(vnc_api, job_execution_id,
                                  ImportServerStatus.COMPLETE):
            return ImportServerStatus.COMPLETE
        if self._check_job_status(vnc_api, job_execution_id,
                                  ImportServerStatus.FAILED):
            return ImportServerStatus.FAILED

        return ImportServerStatus.IN_PROGRESS
    # end _get_job_status
    ...
    def is_job_done(self):
        if self._job_status == ImportServerStatus.COMPLETE or \
                self._job_status == ImportServerStatus.FAILED:
            return True
        return False
    # end is_job_done
    ...
    def _get_vnc_api(api_config):
        return VncApi(
            ...
            domain_name=api_config.get('domain_name'),
...
def main():
    ...
    yaml_file = open('node.yaml', 'rb')
    file_read = yaml_file.read()
    ...
    job_template_fq_name = [
        'default-global-system-config', 'server_import_template']
    ...
    test_sd_obj = ImportServerJob(job_template_fq_name,
                                  job_input,
