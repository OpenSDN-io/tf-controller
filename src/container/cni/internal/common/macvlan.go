...
func (intf *MacVlan) ensureVlanIntf() (*netlink.Link, error) {
	...
	vlanIntf := &netlink.Vlan{
		...
		VlanId:       intf.vlanId,
		VlanProtocol: netlink.VLAN_PROTOCOL_UNKNOWN,
	}
	...
	err := ns.WithNetNSPath(intf.containerNamespace,
		func(hostNS ns.NetNS) error {
			...
			if err != nil {
				...
				log.Infof("Temporary interface %s not present inside container's ns %s",
					intf.containerIfName, intf.containerNamespace)
				return nil
			}
...
func buildVlanIfName(str string, _ int) string {
