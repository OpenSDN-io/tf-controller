...
func (vrouter *VRouter) doOp(op, page, containerNameUuid, vmiUuid string,
	...
	if err != nil {
		log.Errorf("Error creating http Request. Op %s Url %s Msg %s. Error : %+v",
			op, url, string(msg), err)
		return nil, err
	}
...
func MakeCniResult(ifname string, vrouterResult *Result) *current.Result {
	...
	intf := &current.Interface{Name: ifname, Mac: vrouterResult.Mac, Mtu: common.CNI_MTU}
...
func (vrouter *VRouter) addVmFile(addMsg []byte, vmiUuid string) error {
	...
	err := os.WriteFile(fname, addMsg, 0644)
	if err != nil {
